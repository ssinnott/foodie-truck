...
const DEFAULTS = Object.freeze({ rest: [0, 1], stiffness: 0.14, damping: 0.7, gain: 2.2, rotGain: 0.6, follow: 0.35, maxAng: 55, teleport: 60 });

/**
 * Get (or lazily create) a named chain on a rig.
 * @param {object} rig from buildRig
 * @param {string} name chain id
 * @param {number} n segment count
 * @param {{ joint?: string, rest?: [number, number], stiffness?: number, damping?: number, gain?: number, rotGain?: number, follow?: number, maxAng?: number }} [opts]
 ...
 */
export function getChain(rig, name, n = 3, opts = null) {
  ...
  ch = rig.chains[name] = {
    ...
    total(i) { let s = 0; for (let k = 0; k <= i && k < this.n; k++) s += this.ang[k]; return s; },
  };
  return ch;
}
 ...
 */
export function stepChain(ch, dx, dy, dAng) {
...
export function resetChain(ch) { ch.ang.fill(0); ch.vel.fill(0); ch.init = false; }

/** Convenience for drawing: rotate the context by segment i's angle (degrees). */
export function chainRotate(ctx, ch, i) { ctx.rotate(rad(ch.ang[i])); }
