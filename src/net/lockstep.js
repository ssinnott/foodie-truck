...
/**
 * @param {{ localSlot: number, players?: number, delay?: number, redundancy?: number, checksumEvery?: number }} o
 */
export function createLockstep({ localSlot, players = 2, delay = 3, redundancy = 8, checksumEvery = 30 }) {
  ...
  const localSums = new Map();   // frame -> our checksum (bounded; kept for every peer to compare against)
  const pendingSums = new Map(); // frame -> Map(slot -> their checksum) that arrived before ours
  ...
  const gone = (slot, frame) => dropAt[slot] >= 0 && frame >= dropAt[slot];
  const has = (slot, frame) => gone(slot, frame) || stamp[slot][frame & MASK] === frame;
  const get = (slot, frame) => (!gone(slot, frame) && stamp[slot][frame & MASK] === frame ? masks[slot][frame & MASK] : 0);
  ...
  function compare(frame, slot, mine, theirs) {
