...
function needleAt(sweep) { return sweep <= SWEEP_N ? sweep * SWEEP_STEP : (SWEEP_N * 2 - sweep) * SWEEP_STEP; }
...
function clockIcon(ctx, x, y) { drawFood(ctx, 'carrot', x, y, 4); }
...
export class GardenScreen extends Screen {
  constructor(game) { super(game, 'garden'); this.seats = []; this.tops = []; }
  ...
  enter(params) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      s.rig.weapon = GARDEN_TRUG; s.rig.trugCount = 0;
